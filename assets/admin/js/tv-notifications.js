window.ElonixNotifier = (function($) {
    function showToast(message, type = 'success') {
        ...
        const $toast = $('<div class="tv-toast tv-toast-' + type + '"><span class="dashicons ' + iconClass + '"></span><span class="tv-toast-message">' + message + '</span></div>');
    ...
    function showModal(options) {
        ...
        let detailsHtml = '';
        if (options.details) {
            detailsHtml = '<div class="tv-notifier-details"><div class="tv-notifier-details-toggle">Show Technical Details <span class="dashicons dashicons-arrow-down-alt2"></span></div><div class="tv-notifier-details-content" style="display:none;"><pre>' + options.details + '</pre></div></div>';
        }
        ...
        const $modal = $(`
            <div class="tv-notifier-overlay">
                <div class="tv-notifier-dialog">
                    <div class="tv-notifier-header">
                        ${iconHtml}
                        <h2>${options.title}</h2>
                    </div>
                    <div class="tv-notifier-body">
                        <p>${options.message}</p>
                        ${detailsHtml}
                    </div>
                    <div class="tv-notifier-footer">
                        ${buttonsHtml}
                    </div>
                </div>
            </div>
        ...
        $modal.find('.tv-notifier-details-toggle').on('click', function() {
            $modal.find('.tv-notifier-details-content').slideToggle();
        });

        $modal.find('.tv-notifier-footer button').on('click', function() {

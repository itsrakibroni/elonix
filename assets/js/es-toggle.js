...
(function($) {
	...
	var ESToggleHandler = elementorModules.frontend.handlers.Base.extend({
		
		getDefaultSettings: function() {
			return {
				selectors: {
					toggleWrapper: '.es-toggle__wrapper',
					toggleList: '.es-toggle__list',
					toggleItem: '.es-toggle__item',
					toggleButton: '.es-toggle__button'
				},
				classes: {
					activeButton: 'es-toggle__button--active',
					stateHidden: 'es-state-is-hidden',
					stateVisible: 'es-state-is-visible',
					stateInactive: 'es-state-is-inactive',
					stateActive: 'es-state-is-active'
				},
				keyboard: {
					ENTER: 13,
					SPACE: 32,
				...
				constants: {
					events: {
						INIT: 'esToggle:init',
						BEFORE_TOGGLE: 'esToggle:beforeToggle',
						AFTER_TOGGLE: 'esToggle:afterToggle',
						DESTROY: 'esToggle:destroy',
						REFRESH: 'esToggle:refresh',
						SYNC: 'esToggle:sync',
						ERROR: 'esToggle:error'
					},
					states: {
						IDLE: 'IDLE',
						READY: 'READY',
		...
		prepareSyncGroup: function() {
			...
			if (!ESToggleHandler.syncGroups) ESToggleHandler.syncGroups = {};
			if (!ESToggleHandler.syncGroups[this.syncGroup]) ESToggleHandler.syncGroups[this.syncGroup] = [];
			
			ESToggleHandler.syncGroups[this.syncGroup].push(this);
		},
		
		clearSyncGroup: function() {
			if (this.syncGroup && ESToggleHandler.syncGroups && ESToggleHandler.syncGroups[this.syncGroup]) {
				var idx = ESToggleHandler.syncGroups[this.syncGroup].indexOf(this);
				if (idx > -1) {
					ESToggleHandler.syncGroups[this.syncGroup].splice(idx, 1);
				}
			}
		},
		...
		resolveInitialState: function() {
			...
			if (this.toggleSettings.remember_state === 'session_storage' && sessionStorage) {
				var stored = sessionStorage.getItem('esToggle_' + this.$element.data('id'));
				if (stored !== null) indexToActivate = parseInt(stored, 10);
			} else if (this.toggleSettings.remember_state === 'local_storage' && localStorage) {
				var stored = localStorage.getItem('esToggle_' + this.$element.data('id'));
				if (stored !== null) indexToActivate = parseInt(stored, 10);
			}
			...
			if (indexToActivate === -1 && this.toggleSettings.enable_url_hash === 'yes') {
				...
				if (hash) {
					var $btn = this.elements.$toggleButtons.filter('[data-es-target="' + hash + '"]');
					if ($btn.length) {
						indexToActivate = $btn.data('es-index');
					}
				}
			}
			...
			if (indexToActivate === -1 && mode === 'query_parameter') {
				...
				var param = urlParams.get('es_toggle');
				if (param) {
					var $btn = this.elements.$toggleButtons.filter('[data-es-target="#' + param + '"], [data-es-target=".' + param + '"]');
					if ($btn.length) {
						indexToActivate = $btn.data('es-index');
					}
				}
		...
		emitError: function(code, message, selector) {
			...
			if (this.canTransition(states.ERROR)) {
				...
				if (this.toggleSettings && this.toggleSettings.debug_mode === 'yes' && elementorFrontend.isEditMode()) {
					console.error('[ES Toggle Error] ' + code + ': ' + message + (selector ? ' (' + selector + ')' : ''));
				}
		...
		_activateInternal: function(index) {
			...
			var $btn = this.buttonCache[index] || this.elements.$toggleButtons.filter('[data-es-index="' + index + '"]');
			...
			var selector = this.selectorCache[index] || $btn.data('es-target');
				...
				for (var i = 0; i < targets.length; i++) {
					promises.push(new Promise(function(resolve) {
						...
						if (self.transitionEffect === 'none') {
							...
							node.classList.add('es-transition-enabled', 'es-anim-' + self.transitionEffect + '-enter');
							node.classList.remove(classes.stateHidden);
							node.classList.add(classes.stateVisible);
							
							requestAnimationFrame(function() {
								requestAnimationFrame(function() {
									node.classList.add('es-anim-' + self.transitionEffect + '-enter-active');
								});
							});
							...
							Promise.race([transitionPromise, fallbackPromise]).then(function() {
								node.classList.remove('es-transition-enabled', 'es-anim-' + self.transitionEffect + '-enter', 'es-anim-' + self.transitionEffect + '-enter-active');
								resolve();
							});
						}
					}));
				...
				for (var i = 0; i < targets.length; i++) {
					promises.push(new Promise(function(resolve) {
						...
						if (self.transitionEffect === 'none') {
							...
							node.classList.add('es-transition-enabled', 'es-anim-' + self.transitionEffect + '-exit');
							...
							Promise.race([transitionPromise, fallbackPromise]).then(function() {
								node.classList.remove('es-transition-enabled', 'es-anim-' + self.transitionEffect + '-exit');
								if (isEditor && isHelperEnabled) {
									node.classList.add(classes.stateInactive);
									node.classList.remove(classes.stateHidden);
								} else {
		...
		saveState: function(index) {
			if (this.toggleSettings.remember_state === 'session_storage' && sessionStorage) {
				sessionStorage.setItem('esToggle_' + this.$element.data('id'), index);
			} else if (this.toggleSettings.remember_state === 'local_storage' && localStorage) {
				localStorage.setItem('esToggle_' + this.$element.data('id'), index);
			}
		...
		refreshGroup: function() {
			if (!this.syncGroup || !ESToggleHandler.syncGroups[this.syncGroup]) return;
			var members = ESToggleHandler.syncGroups[this.syncGroup];
			this.groupMembers = members.filter(function(m) { return !m.isState('DESTROYED'); });
			ESToggleHandler.syncGroups[this.syncGroup] = this.groupMembers;
		},
		...
		bindEvents: function() {
			...
			this.elements.$toggleButtons.on('click.esToggle', function(e) {
				e.preventDefault();
				var index = $(this).data('es-index');
				self.toggle(index);
			});

			this.elements.$toggleButtons.on('keydown.esToggle', function(e) {
			...
			if (elementorFrontend.isEditMode()) {
				var namespace = '.esToggleEditor_' + this.$element.data('id');
				$('body').on('click' + namespace, '.es-state-is-inactive', function(e) {
		...
		unbindEvents: function() {
			this.elements.$toggleButtons.off('.esToggle');
			if (elementorFrontend.isEditMode()) {
				var namespace = '.esToggleEditor_' + this.$element.data('id');
				$('body').off(namespace);
			}
		...
		updateIndicator: function() {
			var $indicator = this.$element.find('.es-toggle__indicator');
			...
			var list = this.$element.find('.es-toggle__list')[0];
			var btn = $btn[0];
		...
		cacheTargets: function() {
			...
			this.elements.$toggleButtons.each(function(index, element) {
				var $button = $(element);
				var targetSelector = $button.data('es-target');
				...
				if (targetSelector && !self.targetCache[targetSelector]) {
					...
					for (var i = 0; i < nodes.length; i++) {
						nodes[i].style.setProperty('--es-toggle-duration', durationStr);
						nodes[i].style.setProperty('--es-toggle-easing', easingStr);
					}
				}
			});
		...
		refreshTargets: function(selector) {
			...
			for (var i = 0; i < nodes.length; i++) {
				var node = nodes[i];
				node.style.setProperty('--es-toggle-duration', this.transitionDurationStr);
				node.style.setProperty('--es-toggle-easing', this.transitionEasingStr);
			}
			return nodes;
		},
			...
			for (var index in this.selectorCache) {
				...
				if (!nodes || nodes.length === 0) {
					if (isEditor && this.toggleSettings.debug_mode === 'yes' && action === 'editor_warning') {
						console.warn('[ES Toggle] Warning: Target selector "' + selector + '" found no matching elements on this page.');
					}
		...
		hideAllTargets: function() {
			...
			this.elements.$toggleButtons.each(function(index, element) {
				var targetSelector = $(element).data('es-target');
				if (targetSelector) {
	...
	$(window).on('elementor/frontend/init', function() {
		elementorFrontend.hooks.addAction('frontend/element_ready/es-toggle.default', function($element) {
			elementorFrontend.elementsHandler.addHandler(ESToggleHandler, {
				$element: $element
			});
		});
	});

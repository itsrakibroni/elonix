...
jQuery(window).on('elementor/frontend/init', () => {
	elementorFrontend.hooks.addAction('frontend/element_ready/es-accordion.default', ($element) => {
		const accordion = $element.find('.es-accordion-container')[0];
		if (!accordion) return;
		...
		const items    = accordion.querySelectorAll('.es-accordion-item');
		const triggers = accordion.querySelectorAll('.es-accordion-header-trigger');

		triggers.forEach((trigger, index) => {
			const item  = items[index];
			const panel = item.querySelector('.es-accordion-panel');
			...
			trigger.addEventListener('focus', () => {
				trigger.closest('.es-accordion-header').classList.add('es-focus');
			});
			trigger.addEventListener('blur', () => {
				trigger.closest('.es-accordion-header').classList.remove('es-focus');
			});
		...
		function toggleItem(index) {
			...
			const currentPanel = currentItem.querySelector('.es-accordion-panel');
			const isExpanded = currentTrigger.getAttribute('aria-expanded') === 'true';

			if (isExpanded) {
				slideUp(currentPanel, duration, () => {
					currentItem.classList.remove('es-active');
				...
				if (!allowMultiple) {
					items.forEach((item, idx) => {
						if (idx !== index && item.classList.contains('es-active')) {
							const otherTrigger = triggers[idx];
							const otherPanel   = item.querySelector('.es-accordion-panel');
				...
				currentItem.classList.add('es-active');

...
(function($) {
	...
	var TVMarqueeHandler = elementorModules.frontend.handlers.Base.extend({
		
		getDefaultSettings: function() {
			return {
				selectors: {
					carousel: '.tv-marquee__carousel',
					track: '.tv-marquee__track'
				}
			};
		},
		...
		initializeMarquee: function() {
			...
			$carousel.addClass('tv-marquee--continuous');
			if (direction === 'up' || direction === 'down') {
				$carousel.addClass('tv-marquee--vertical');
			}
			...
			$track.on('beforeStarting', function() {
				$carousel.trigger('tv_marquee:beforeStarting');
			});
			$track.on('finished', function() {
				$carousel.trigger('tv_marquee:finished');
			});
			$track.on('paused', function() {
				$carousel.trigger('tv_marquee:paused');
			});
			$track.on('resumed', function() {
				$carousel.trigger('tv_marquee:resumed');
			});

			Elonix.Core.InstanceRegistry.register(this.$element.data('id'), marqueeInstance);
			
			// Pause button binding
			var $pauseBtn = this.$element.find('.tv-marquee__pause-btn');
			if ($pauseBtn.length) {
	...
	$(window).on('elementor/frontend/init', function() {
		elementorFrontend.hooks.addAction('frontend/element_ready/tv-marquee.default', function($element) {
			elementorFrontend.elementsHandler.addHandler(TVMarqueeHandler, {
				$element: $element
			});
		});
	});

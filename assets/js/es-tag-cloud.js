...
jQuery(window).on("elementor/frontend/init", () => {
  elementorFrontend.hooks.addAction("frontend/element_ready/es-tag-cloud.default", ($element) => {
    const wrap = $element[0].querySelector(".es-tag-cloud-wrap");
    if (!wrap) {
      return;
    }
    ...
    const tags = wrap.querySelectorAll(".es-tag-item");
    let selectedTerms = [];

    const clearBtnWrap = wrap.querySelector(".es-tag-clear-filter-wrap");
    const clearBtn = clearBtnWrap ? clearBtnWrap.querySelector(".es-tag-clear-filter-btn") : null;

    // Fetch the target es-post-list wrapper element dynamically
    const getTargetPostListWrap = () => {
      const targetEl = document.getElementById(targetWidgetId);
      if (targetEl) {
        return targetEl.querySelector(".es-post-list-wrap");
      }
      const targetClassEl = document.querySelector("." + targetWidgetId);
      if (targetClassEl) {
        return targetClassEl.querySelector(".es-post-list-wrap");
      }
      return null;
    };
    ...
    tags.forEach((tag) => {
      ...
      const handleTagSelection = (e) => {
        ...
        if ("ajax_filter" === interaction) {
          ...
          if (tag.classList.contains("es-active")) {
            tag.classList.remove("es-active");
            tag.setAttribute("aria-pressed", "false");
            selectedTerms = [];
          } else {
            // Clean active classes on sibling elements
            tags.forEach((t) => {
              t.classList.remove("es-active");
              t.setAttribute("aria-pressed", "false");
            });
            tag.classList.add("es-active");

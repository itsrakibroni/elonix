...
jQuery( window ).on(
	...
	elementorFrontend.hooks.addAction(
		'frontend/element_ready/tv-post-list.default',
		($element) => {
			const wrap = $element[0].querySelector( '.tv-post-list-wrap' );
			if ( ! wrap ) {
				return;
			}
			const container = wrap.querySelector( '.tv-post-list-container' );
			if ( ! container ) {
				return;
			}
        ...
        return;
			...
			if ( currentPage >= maxPages ) {
				const pagWrap = wrap.querySelector( '.tv-post-list-pagination' );
				if ( pagWrap ) {
			...
			const loadMorePosts = () => {
				...
				const btn = wrap.querySelector( '.tv-load-more-btn' );
				const spinner     = btn ? btn.querySelector( '.tv-btn-loading-icon' ) : null;
				const trigger     = wrap.querySelector( '.tv-infinite-scroll-trigger' );
				...
				sk.className = `tv-post-card tv-skeleton-card tv-layout-${settings.layout}`;
				sk.innerHTML = `
					<div class="tv-post-thumbnail tv-skeleton-thumb"></div>
				...
				formData.append( 'action', 'tv_post_list_fetch_posts' );
        ...
        // 3. Setup pagination triggers
				if ( 'load_more' === paginationType ) {
					const loadMoreBtn = wrap.querySelector( '.tv-load-more-btn' );
					if ( loadMoreBtn ) {
					...
					const scrollTrigger = wrap.querySelector( '.tv-infinite-scroll-trigger' );
					if ( scrollTrigger ) {
				...
				// 4. Custom Filter Listener for AJAX Filter widgets (like tv-tag-cloud)
				wrap.addEventListener(
                    ...
                    // Show pagination block if it was hidden
						const paginationWrapper = wrap.querySelector( '.tv-post-list-pagination' );

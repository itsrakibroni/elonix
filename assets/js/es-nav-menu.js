...
jQuery( window ).on(
	...
	() => {
		...
		'frontend/element_ready/es-nav-menu.default',
		($element) => {
			const wrapper = $element[0].querySelector( '.es-nav-menu-wrapper' );
        if ( ! wrapper || wrapper.classList.contains( 'es-nav-menu-initialized' )) {
            return;
        }
			wrapper.classList.add( 'es-nav-menu-initialized' );
			...
			// DOM selectors
				const toggler      = wrapper.querySelector( '.es-nav-menu-hamburger' );
			const closeBtn         = wrapper.querySelector( '.es-nav-menu-mobile-close' );
			const overlay          = wrapper.querySelector( '.es-nav-menu-mobile-overlay' );
			const mobileDrawer     = wrapper.querySelector( '.es-nav-menu-mobile-drawer' );
			const mobileDropdown   = wrapper.querySelector( '.es-nav-menu-mobile-dropdown' );
			const desktopContainer = wrapper.querySelector( '.es-nav-menu-desktop-container' );
			const desktopMenu      = desktopContainer ? desktopContainer.querySelector( '.es-navbar-nav' ) : null;
			// Active drawer check
			...
			/* =========================================================================
				...
				const openMobileMenu = () => {
					wrapper.classList.add( 'es-mobile-menu-active' );
					if (toggler) {
			...
			const closeMobileMenu        = () => {
				wrapper.classList.remove( 'es-mobile-menu-active' );
				if (toggler) {
				...
				if (activeContainer) {
					const openSubmenus = activeContainer.querySelectorAll( '.es-dropdown-open' );
					openSubmenus.forEach(
					(li) => {
						li.classList.remove( 'es-dropdown-open' );
						const sub      = li.querySelector( ':scope > .es-dropdown' );
						if (sub) {
						...
						const subToggle = li.querySelector( '.es-submenu-indicator-mobile-toggle' );
						if (subLink) {
			...
			if (toggler) {
				toggler.addEventListener(
					...
					(e) => {
						e.preventDefault();
						if (wrapper.classList.contains( 'es-mobile-menu-active' )) {
							closeMobileMenu();
						} else {
						openMobileMenu();
                    }
        ...
        if (overlay) {
			...
			// Close menu on Escape key press
				window.addEventListener(
					'keydown',
					(e) => {
						if (e.key === 'Escape' && wrapper.classList.contains( 'es-mobile-menu-active' )) {
							closeMobileMenu();
						}
					}
				);
        ...
        if ( ! activeContainer) {
            return;
					...
					const dropdownItems     = activeContainer.querySelectorAll( '.es-dropdown-has' );
					dropdownItems.forEach(
                (li) => {
						const link      = li.querySelector( ':scope > a' );
						const submenu   = li.querySelector( ':scope > .es-dropdown' );
						if ( ! link || ! submenu) {
							return;
						}
						// Generate unique ID for submenu if not exists
						if ( ! submenu.id) {
							submenu.id = 'es-submenu-' + Math.random().toString( 36 ).substr( 2, 9 );
						}
							submenu.setAttribute( 'aria-hidden', 'true' );
						// Ensure caret exists and wrap inside mobile-toggle if it exists
							let indicator = link.querySelector( '.es-submenu-indicator' );
						let toggleBtn         = li.querySelector( '.es-submenu-indicator-mobile-toggle' );
						if ( ! toggleBtn) {
							toggleBtn           = document.createElement( 'button' );
							toggleBtn.className = 'es-submenu-indicator-mobile-toggle';
                ...
                toggleBtn.setAttribute( 'aria-expanded', 'false' );
						...
						const isClickOnIcon = activeContainer.classList.contains( 'es-submenu-click-on-icon' );
						// Setup accordion toggle method
							const toggleSubmenu = (itemLi, itemSubmenu, event) => {
                event.preventDefault();
                event.stopPropagation();
                const isOpen    = itemLi.classList.contains( 'es-dropdown-open' );
                // Close other sibling dropdowns on same depth
								const siblings = itemLi.parentNode.querySelectorAll( ':scope > .es-dropdown-has' );
                siblings.forEach(
								(sib) => {
									if (sib !== itemLi && sib.classList.contains( 'es-dropdown-open' )) {
										sib.classList.remove( 'es-dropdown-open' );
										const sibSub = sib.querySelector( ':scope > .es-dropdown' );
										if (sibSub) {
										...
										const sibToggle = sib.querySelector( '.es-submenu-indicator-mobile-toggle' );
										if (sibLink) {
                ...
                // Entire Link Item Click Mode
							link.addEventListener(
								...
								if (isHash) {
									...
									if ( ! li.classList.contains( 'es-dropdown-open' )) {
										toggleSubmenu( li, submenu, e );
									}
								}
								}
			...
			if (desktopMenu && triggerType === 'click') {
				const dropdownItems = desktopMenu.querySelectorAll( '.es-dropdown-has' );
				...
				dropdownItems.forEach(
                    ...
                    link.addEventListener(
							...
							const isOpen = li.classList.contains( 'es-dropdown-open' );
							if (isHash || ! isOpen) {
								e.preventDefault();
								e.stopPropagation();

								// Close open siblings at this level
								const siblings = li.parentNode.querySelectorAll( ':scope > .es-dropdown-has' );
								siblings.forEach(
									(sib) => {
                                    if (sib !== li) {
                                        sib.classList.remove( 'es-dropdown-open' );
								...
								if (isOpen) {
									...
									li.classList.add( 'es-dropdown-open' );
									link.setAttribute( 'aria-expanded', 'true' );
								}
							}
								}
			...
			if (desktopMenu) {
				...
				rootLinks.forEach(
					...
					const submenu = rootLink.parentElement.querySelector( ':scope > .es-dropdown' );
					if (submenu) {
						const subLinks = submenu.querySelectorAll( 'li > a' );
						subLinks.forEach(
                            (subLink, subIndex) => {
							subLink.addEventListener(
                                ...
                                subLinks[prevSub].focus();
											...
											rootLink.parentElement.classList.remove( 'es-dropdown-open' );
        ...
        if (stickyAnimation !== 'none') {
            ...
            className => {
                if (className.startsWith( 'es-sticky-effect-' )) {
            ...
            document.body.classList.add( `es-sticky-effect-${stickyAnimation}` );
            ...
            if (duration) {
                applyTarget.style.setProperty( '--es-sticky-duration', `${duration}ms` );
            }
            if (delay) {
                applyTarget.style.setProperty( '--es-sticky-delay', `${delay}ms` );
            }
            if (easing) {
                applyTarget.style.setProperty( '--es-sticky-easing', easing );
            }
        }
			/* =========================================================================
			...
			window.addEventListener(
                ...
                () => {
						...
						if (scrollTop > lastScrollTop && scrollTop > 50) {
							// Scrolling Down
							document.body.classList.remove( 'es-scroll-up' );
							document.body.classList.add( 'es-scroll-down' );
							wrapper.classList.remove( 'es-scroll-up' );
							wrapper.classList.add( 'es-scroll-down' );
						} else {
                // Scrolling Up
							document.body.classList.remove( 'es-scroll-down' );
                document.body.classList.add( 'es-scroll-up' );
                wrapper.classList.remove( 'es-scroll-down' );
                wrapper.classList.add( 'es-scroll-up' );
						}

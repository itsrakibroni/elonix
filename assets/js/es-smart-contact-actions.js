...
class ElonixSmartContactActions {
  ...
  constructor(container) {
    ...
    this.trigger = container.querySelector(".es-sca-trigger-btn");
    this.itemsList = container.querySelector(".es-sca-item-list");
    this.drawer = container.querySelector(".es-sca-card-drawer");
  ...
  bindEvents() {
    ...
    const qrModal = this.container.querySelector(".es-sca-qr-modal");
    if (qrModal) {
      const closeBtn = qrModal.querySelector(".es-sca-qr-close");
      if (closeBtn) {
    ...
    const actionLinks = this.container.querySelectorAll(".es-sca-item-btn, .es-sca-agent-link-btn, .es-sca-trigger-btn[href]");
    ...
    document.addEventListener("click", (e) => {
      if (!this.container.contains(e.target)) {
        const overlay = document.querySelector(".es-sca-qr-overlay");
  ...
  updateStatusIndicators() {
    ...
    const indicators = this.container.querySelectorAll(".es-sca-status-indicator");

    indicators.forEach((indicator) => {
      indicator.classList.remove("es-sca-status-online", "es-sca-status-offline", "es-sca-status-away", "es-sca-status-lunch");
      let title = "Offline";
      if (status === "online") {
        indicator.classList.add("es-sca-status-online");
        title = "Online";
      } else if (status === "lunch") {
        indicator.classList.add("es-sca-status-lunch");
        title = "Lunch Break / Away";
      } else {
        indicator.classList.add("es-sca-status-offline");
        title = "Offline";
      }
  ...
  initRadialLayout() {
    ...
    const items = list.querySelectorAll(".es-sca-item");
    ...
    items.forEach((item, index) => {
      ...
      item.style.setProperty("--es-sca-radial-x", `${x}px`);
      item.style.setProperty("--es-sca-radial-y", `${y}px`);
      item.style.left = "0";
      item.style.top = "0";
    });
  }
  ...
  toggleMenu() {
    if (this.config.layout === "speed_dial" || this.config.layout === "radial") {
      const isOpen = this.container.classList.contains("es-sca-dial-open");
      if (isOpen) {
        this.closeAllMenus();
      } else {
        if (this.itemsList) {
        ...
        this.container.classList.add("es-sca-dial-open");
        this.trigger.setAttribute("aria-expanded", "true");
        this.focusTrap(this.itemsList, this.trigger);
      }
    } else if (this.config.layout === "contact_card") {
      const isOpen = this.container.classList.contains("es-sca-card-open");
      if (isOpen) {
        this.closeAllMenus();
      } else {
        if (this.drawer) {
        ...
        this.container.classList.add("es-sca-card-open");
        this.trigger.setAttribute("aria-expanded", "true");
   ...
   */
  closeAllMenus() {
    if (this.container.classList.contains("es-sca-dial-open")) {
      this.container.classList.remove("es-sca-dial-open");
      if (this.trigger) {
    ...
    if (this.container.classList.contains("es-sca-card-open")) {
      this.container.classList.remove("es-sca-card-open");
      if (this.trigger) {
        this.trigger.setAttribute("aria-expanded", "false");
      }
  ...
  handleContactClick(e, itemBtn) {
    ...
    const parentItem = itemBtn.closest(".es-sca-item, .es-sca-agent-row");
    const href = itemBtn.getAttribute("href") || "#";
  ...
  loadQrLibrary(callback) {
    ...
    const currentScript = document.querySelector('script[src*="es-smart-contact-actions"]');
    script.src = currentScript ? currentScript.src.replace('es-smart-contact-actions.js', 'vendor/qrious.min.js') : "/wp-content/plugins/elonix/assets/js/vendor/qrious.min.js";
    script.async = true;
    script.onload = () => {
      callback();
    };
  ...
  openQrModal(qrData) {
    this.loadQrLibrary(() => {
      const modal = this.container.querySelector(".es-sca-qr-modal");
      if (!modal) {
        return;
      }
      const canvas = modal.querySelector(".es-sca-qr-canvas");
      if (!canvas) {
        return;
      }
      try {
      ...
      let overlay = document.querySelector(".es-sca-qr-overlay");
      if (!overlay) {
        overlay = document.createElement("div");
        overlay.className = "es-sca-qr-overlay";
        document.body.appendChild(overlay);
      }
      overlay.style.display = "block";
      modal.style.display = "flex";
      modal.offsetHeight;
      modal.classList.add("es-sca-qr-active");
  ...
  closeQrModal() {
    ...
    const overlay = document.querySelector(".es-sca-qr-overlay");
    if (modal) {
      modal.classList.remove("es-sca-qr-active");
      setTimeout(() => {
        ...
        if (!this.scrollTicking) {
          window.requestAnimationFrame(() => {
            ...
            if (currentScrollY > this.lastScrollY && currentScrollY > 100) {
              this.container.classList.add("es-sca-hidden");
            } else {
              // Scroll up shows
              this.container.classList.remove("es-sca-hidden");
            }
...
const esInitSmartContactActions = () => {
  const widgets = document.querySelectorAll(".es-sca-container");
...
if (document.readyState === "loading") {
  document.addEventListener("DOMContentLoaded", esInitSmartContactActions);
} else {
  esInitSmartContactActions();
}

// Elementor Preview registration
const esRegisterElementorSmartContactActions = () => {
  if (window.elementorFrontend && elementorFrontend.hooks) {
    elementorFrontend.hooks.addAction("frontend/element_ready/es-smart-contact-actions.default", ($scope) => {
      if (!$scope || !$scope.length) {
        return;
      }
      const target = $scope[0].querySelector(".es-sca-container");
...
if (window.elementorFrontend) {
  esRegisterElementorSmartContactActions();
} else {
  window.addEventListener("elementor/frontend/init", esRegisterElementorSmartContactActions);
}

/**
 * Elonix Back To Top Widget Javascript Engine (tv-back-to-top)
 *
...
class ElonixBackToTop {
	...
	constructor(container) {
		...
		this.button    = container.querySelector( '.tv-back-to-top-button' );
		...
		this.svgFill     = container.querySelector( '.tv-progress-ring-fill' );
		this.percentText = container.querySelector( '.tv-progress-percent-text' );
		this.readingText = container.querySelector( '.tv-reading-time-text' );
		this.barTrack    = container.querySelector( '.tv-progress-track' );
		this.barFill     = container.querySelector( '.tv-progress-fill' );
	...
	evaluateVisibility(scrollTop, progress) {
		...
		if (shouldShow) {
			if ( ! this.isVisible) {
				this.container.classList.add( 'tv-visible' );
				if (this.button) {
			...
			if (this.isVisible) {
				this.container.classList.remove( 'tv-visible' );
				if (this.button) {
	...
	renderProgress(progress) {
		...
		if (this.button && this.config.iconSwap) {
			if (progress >= 0.99) {
				this.button.classList.add( 'tv-complete' );
			} else {
				this.button.classList.remove( 'tv-complete' );
			}
		}
	}
	...
	triggerInactivityTimer() {
		...
		this.container.classList.remove( 'tv-inactive' );

		this.inactivityTimer = setTimeout(
			() => {
				if (this.isVisible) {
					this.container.classList.add( 'tv-inactive' );
				}
			},
	...
	handleFooterAvoidance(viewportHeight) {
		...
		if (footerTop < viewportHeight) {
			...
			if (this.container.classList.contains( 'tv-align-center' )) {
...
const tvInitBackToTop = () => {
	const items       = document.querySelectorAll( '.tv-back-to-top-container, .tv-progress-bar-track' );
	items.forEach(
		item => {
...
if (document.readyState === 'loading') {
	document.addEventListener( 'DOMContentLoaded', tvInitBackToTop );
} else {
	tvInitBackToTop();
}

// Elementor Editor preview dynamic handler registration
const tvRegisterElementorBackToTop = () => {
	if (window.elementorFrontend && elementorFrontend.hooks) {
		elementorFrontend.hooks.addAction(
			'frontend/element_ready/tv-back-to-top.default',
			($scope) => {
            if ( ! $scope || ! $scope.length) {
                return;
            }
				const target = $scope[0].querySelector( '.tv-back-to-top-container, .tv-progress-bar-track' );
...
if (window.elementorFrontend) {
	tvRegisterElementorBackToTop();
} else {
	window.addEventListener( 'elementor/frontend/init', tvRegisterElementorBackToTop );
}

...
class ElonixStickyEngine {
	...
	getConfig($scope) {
		...
		if (elementorFrontend.isEditMode()) {
			...
			return {
				devices: settings.es_sticky_devices || ['desktop', 'tablet', 'mobile'],
				trigger: settings.es_sticky_trigger || 'scroll_down',
				position: settings.es_sticky_position || 'sticky',
				hideOnScroll: settings.es_sticky_hide_on_scroll === 'yes'
			};
		} else {
			if (!node.hasAttribute('data-es-sticky-config')) return null;
			try {
				return JSON.parse(node.getAttribute('data-es-sticky-config'));
			} catch (e) {
				return null;
			}
		}
	...
	isStickyEnabled($scope) {
		if (elementorFrontend.isEditMode()) {
			const settings = elementorFrontend.elements.getElementSettings($scope);
			return settings.es_sticky_enable === 'yes';
		}
		return $scope[0].classList.contains('es-sticky-yes');
	}
	...
	init($scope) {
		...
		if (config.position === 'fixed') {
			...
			placeholder.classList.add('es-sticky-placeholder');
	...
	stickElement(item, shouldHide) {
		if (!item.isSticking) {
			...
			if (item.config.position === 'fixed' && item.placeholder) {
				item.el.classList.add('es-sticky-fixed');
				const computedStyle = window.getComputedStyle(item.el);
	...
	resetElement(item) {
		if (item.isSticking) {
			...
			if (item.config && item.config.position === 'fixed' && item.placeholder) {
				item.el.classList.remove('es-sticky-fixed');
				item.el.style.top = '';
...
class ElonixStickyHandler extends elementorModules.frontend.handlers.Base {
	...
	onElementChange(propertyName) {
		if (propertyName.indexOf('es_sticky_') === 0) {

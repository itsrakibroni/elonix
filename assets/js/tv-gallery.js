...
(function($) {
	...
	var TVGalleryHandler = elementorModules.frontend.handlers.Base.extend({
		
		getDefaultSettings: function() {
			return {
				selectors: {
					masonry: '.tv-gallery__masonry',
					grid: '.tv-gallery__grid',
					filterBar: '.tv-gallery__filter-bar',
					filterBtn: '.tv-gallery__filter-btn',
					item: '.tv-gallery__item',
					loadMoreBtn: '.tv-gallery__load-more-btn',
					wrapper: '.tv-gallery'
				}
			};
		},
		...
		bindEvents: function() {
			...
			if (this.elements.$filterBtns.length) {
				this.elements.$filterBtns.on('click', function(e) {
					...
					self.elements.$filterBtns.removeClass('tv-active').attr('aria-pressed', 'false');
					$btn.addClass('tv-active').attr('aria-pressed', 'true');
		...
		onInit: function() {
			...
			$(window).on('resize.tvGallery' + this.$element.data('id'), function() {
		...
		checkLoadMoreState: function() {
			...
			var activeFilter = this.elements.$filterBtns.filter('.tv-active').attr('data-filter') || '*';
			var filterSelector = activeFilter === '*' ? '.tv-gallery__item' : '.tv-gallery__item[data-category="' + activeFilter + '"]';
			...
			$btn.find('.tv-gallery__load-more-text').text($btn.data('default-text'));
			$btn.find('.tv-gallery__spinner').hide();
			$btn.show();
		},
		...
		loadMoreItems: function() {
			...
			$btn.find('.tv-gallery__load-more-text').text($btn.data('loading-text'));
			$btn.find('.tv-gallery__spinner').show();

			var data = {
				action: 'tv_gallery_load_more',
			...
			$.ajax({
				...
				success: function(response) {
					if (response.success && response.data.html) {
						...
						var layoutMode = elementSettings.layout_mode || 'grid';
						var $container = layoutMode === 'masonry' ? self.elements.$masonry.find('.tv-isotope-grid') : self.elements.$grid;
		...
		onDestroy: function() {
			...
			$(window).off('resize.tvGallery' + this.$element.data('id'));
		}
	});

	$(window).on('elementor/frontend/init', function() {
		elementorFrontend.hooks.addAction('frontend/element_ready/tv-gallery.default', function($element) {
			elementorFrontend.elementsHandler.addHandler(TVGalleryHandler, {
				$element: $element
			});
		});
	});

...
jQuery(window).on('elementor/frontend/init', () => {
	elementorFrontend.hooks.addAction('frontend/element_ready/es-categories.default', ($element) => {
		const container = $element[0].querySelector('.es-categories-wrapper');
		if ( ! container ) {
			return;
		}

		// 1. Initialize Swiper Carousel if layout is carousel
		if ( container.classList.contains('es-categories-style-carousel') ) {
			const swiperContainer = container.querySelector('.es-categories-swiper');
			...
			if ( swiperContainer && optionsAttr ) {
				try {
					...
					if ( paginationEl ) {
						...
						if ( paginationEl.classList.contains('es-pag-fraction') ) {
							pagOpt.type = 'fraction';
						} else if ( paginationEl.classList.contains('es-pag-progress') ) {
							pagOpt.type = 'progressbar';
						} else if ( paginationEl.classList.contains('es-pag-numbers') ) {
							pagOpt.type = 'bullets';
							pagOpt.renderBullet = (index, className) => {
								return `<span class="${className} es-swiper-pagination-number" style="display:inline-flex; width:24px; height:24px; align-items:center; justify-content:center; font-size:11px; margin:0 3px; cursor:pointer; border-radius:50%; background:rgba(0,0,0,0.06); color:#333; line-height:1;">${index + 1}</span>`;
							};
						} else {
							pagOpt.type = 'bullets';
						}
		...
		const accordionButtons = container.querySelectorAll('.es-cat-toggle-btn');
		accordionButtons.forEach((btn) => {
			btn.addEventListener('click', (e) => {
				...
				const collapsedIcon = btn.querySelector('.es-cat-icon-collapsed');
				const expandedIcon = btn.querySelector('.es-cat-icon-expanded');

				if ( isExpanded ) {
					// Collapse nested tree
					targetList.classList.add('es-cat-children-hidden');
					...
					// Expand nested tree
					targetList.classList.remove('es-cat-children-hidden');
		...
		if ( treeWrapper ) {
			const treeItems = Array.from(treeWrapper.querySelectorAll('.es-cat-child-link, .es-cat-title-link'));
			...
			treeItems.forEach((link, idx) => {
				link.addEventListener('keydown', (e) => {
					...
					if ( e.key === 'ArrowDown' ) {
						...
						if ( link.classList.contains('es-cat-title-link') ) {
							const card = link.closest('.es-cat-card-classic, .es-cat-card');
							if ( card ) {
								const toggleBtn = card.querySelector('.es-cat-toggle-btn');
								const childList = card.querySelector('.es-cat-children-list');
								if ( toggleBtn && childList ) {
									...
									if ( ! isExpanded ) {
										...
										const firstChild = childList.querySelector('.es-cat-child-link');
										if ( firstChild ) {
											e.preventDefault();
											firstChild.focus();
										}
						...
						} else if ( link.classList.contains('es-cat-child-link') ) {
							const card = link.closest('.es-cat-card-classic, .es-cat-card');
							if ( card ) {
								const parentLink = card.querySelector('.es-cat-title-link');
								if ( parentLink ) {
									e.preventDefault();
									parentLink.focus();
								}

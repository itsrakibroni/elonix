...
(function($) {
	...
	var ESProgressHandler = elementorModules.frontend.handlers.Base.extend({
		
		getDefaultSettings: function() {
			return {
				selectors: {
					progress: '.es-progress',
					counter: '.es-progress__value-number, .es-progress__marker-val',
					circleFill: '.es-progress__circle-fill'
				},
				classes: {
		...
		initSVGCalculations: function() {
			...
			this.elements.$circleFills.each(function() {
				...
				var isSemi = self.elements.$progress.hasClass('es-progress--semi-circle');
				...
				$fill.css('--es-progress-dasharray', circumference);
				
				var percent = parseFloat($fill.data('percentage')) || 0;
				var offset = circumference - (percent / 100) * circumference;
				$fill.css('--es-progress-dashoffset', offset);
			});
		},
	...
	$(window).on('elementor/frontend/init', function() {
		elementorFrontend.hooks.addAction('frontend/element_ready/es-progress.default', function($element) {
			elementorFrontend.elementsHandler.addHandler(ESProgressHandler, {
				$element: $element
			});
		});
	});

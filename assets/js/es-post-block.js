...
jQuery(window).on("elementor/frontend/init", () => {
  const initTvPostBlock = ($element) => {
    const wrap = $element[0].querySelector(".es-post-block-wrap");
    if (!wrap) {
      return;
    }
    const container = wrap.querySelector(".es-post-block-container");
    if (!container) {
      return;
    }
    ...
    const srAnnouncer = wrap.querySelector(".es-sr-announcer");
    ...
    const fetchPostsData = (isAppend = false) => {
      ...
      const loadMoreBtn = wrap.querySelector(".es-post-block-load-more");
      const loadMoreSpinner = loadMoreBtn ? loadMoreBtn.querySelector(".es-btn-spinner") : null;
      ...
      for (let i = 0; i < skeletonsCount; i++) {
        ...
        sk.className = `es-post-block-card es-skeleton-card es-layout-${layout}`;
        sk.innerHTML = `
						<div class="es-skeleton-img"></div>
						<div class="es-post-block-content">
        ...
        .then((res) => {
          ...
          if (res.success && res.data) {
            ...
            if (isAppend) {
              ...
              container.innerHTML = htmlContent || ` <div class = "es-no-posts" > ${i18n.no_posts} </div> `;
            }
    ...
    const updateLoadMoreTriggerStates = () => {
      const paginationWrap = wrap.querySelector(".es-post-block-pagination");
      if (!paginationWrap) {
        return;
      }
      if (maxPages <= 1) {
    ...
    const updateNumericPaginationMarkup = () => {
      const nav = wrap.querySelector(".es-post-block-pagination-nav");
      if (!nav) {
        return;
      }
      nav.innerHTML = "";
      ...
      prevBtn.className = `es-page-navprev ${currentPage <= 1 ? "disabled" : ""}`;
      if (currentPage <= 1) {
      ...
      for (let i = 1; i <= maxPages; i++) {
        ...
        btn.className = `es-page-num ${i === currentPage ? "active" : ""}`;
        if (i === currentPage) {
      ...
      nextBtn.className = `es-page-navnext ${currentPage >= maxPages ? "disabled" : ""}`;
    ...
    const tabs = wrap.querySelectorAll(".es-filter-tab");
    if (tabs.length > 0) {
      tabs.forEach((tab) => {
    ...
    const searchInput = wrap.querySelector(".es-post-block-search-input");
    if (searchInput) {
    ...
    if (paginationType === "load_more") {
      ...
      const scrollTrigger = wrap.querySelector(".es-post-block-scroll-trigger");
      if (scrollTrigger) {
  ...
  elementorFrontend.hooks.addAction("frontend/element_ready/es-post-block.default", initTvPostBlock);
  elementorFrontend.hooks.addAction("frontend/element_ready/es-search-results.default", initTvPostBlock);
});

...
class Elonix_Toolkit_Search_Results_Query_Helper {
	...
	public static function build_query_args( $settings ) {
		...
		return apply_filters( 'elonix_tv_search_results_query_args', $args, $settings );
	}
	...
	private static function get_post_rating( $post_id ) {
		$keys = array( 'rating', '_rating', 'average_rating', '_average_rating', 'tv_rating' );
		foreach ( $keys as $key ) {
	...
	public static function highlight_keyword( $text, $keyword ) {
		...
		return preg_replace_callback(
			...
			static function ( $matches ) {
				return '<mark class="tv-search-results-highlight">' . esc_html( $matches[0] ) . '</mark>';
			},
			esc_html( $text )
		);
	}

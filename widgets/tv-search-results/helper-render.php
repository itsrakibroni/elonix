...
class Elonix_Toolkit_Search_Results_Render_Helper {
	...
	public static function render_filters( $settings ) {
		...
		?>
		<aside class="tv-search-results-sidebar" aria-label="<?php esc_attr_e( 'Search result filters', 'elonix' ); ?>">
			<button type="button" class="tv-search-results-filter-toggle" aria-expanded="false">
				<?php esc_html_e( 'Filters', 'elonix' ); ?>
			</button>
			<div class="tv-search-results-filter-panel">
				<label>
					<span><?php esc_html_e( 'Post Type', 'elonix' ); ?></span>
					<select class="tv-search-results-filter" data-filter="post_type">
				...
				<label>
					...
					<select class="tv-search-results-filter" data-filter="date_filter">
	...
	public static function render_card( $item, $settings, $index = 0 ) {
		...
		$classes = array( 'tv-search-results-card', 'tv-search-results-card-' . $layout );
		...
		<article class="<?php echo esc_attr( implode( ' ', $classes ) ); ?>">
			...
			foreach ( $element_order as $block ) {
				...
				if ( ! $content_open ) {
					echo '<div class="tv-search-results-card-content">';
					$content_open = true;
				}
				self::render_element_block( $type, $item, $settings, $block );
			}
	...
	private static function render_element_block( $type, $item, $settings, $block ) {
		...
		switch ( $type ) {
			case 'featured_image':
				...
				?>
				<a class="tv-search-results-thumb" href="<?php echo esc_url( $item['url'] ); ?>" tabindex="-1" aria-hidden="true">
			...
			case 'category':
				...
				?>
				<div class="tv-search-results-terms">
			...
			case 'badge':
				...
				echo '<span class="tv-search-results-badge">' . esc_html( $badge_text ) . '</span>';
				break;

			case 'post_type':
				echo '<span class="tv-search-results-post-type">' . esc_html( $item['post_type_name'] ) . '</span>';
				break;
			...
			case 'reading_time':
			case 'comments':
				echo '<div class="tv-search-results-meta-single">';
			...
			case 'title':
				...
				?>
				<<?php echo esc_attr( $title_tag ); ?> class="tv-search-results-title">
			...
			case 'excerpt':
				...
				echo '<p class="tv-search-results-excerpt">' . esc_html( $item['excerpt'] ) . '</p>';
				break;
			...
			case 'tags':
				...
				?>
				<div class="tv-search-results-tags">
			...
			case 'button':
				...
				?>
				<a class="tv-search-results-button" href="<?php echo esc_url( $item['url'] ); ?>">
					<?php
			...
			case 'rating':
				...
				printf(
					'<span class="tv-search-results-rating" aria-label="%1$s">%2$s <strong>%3$s</strong></span>',
	...
	private static function render_meta( $item, $settings ) {
		...
		?>
		<div class="tv-search-results-meta">
	...
	private static function render_meta_item( $type, $item ) {
		switch ( $type ) {
			case 'author':
				printf(
					'<span class="tv-search-results-meta-item tv-search-results-meta-author"><a href="%1$s">%2$s</a></span>',
			...
			case 'date':
				echo '<span class="tv-search-results-meta-item tv-search-results-meta-date">' . esc_html( $item['date'] ) . '</span>';
				break;

			case 'reading_time':
				printf(
					'<span class="tv-search-results-meta-item tv-search-results-meta-reading">%s</span>',
					sprintf(
						/* translators: %d: Estimated reading time in minutes. */
			...
			case 'comments':
				printf(
					'<span class="tv-search-results-meta-item tv-search-results-meta-comments"><a href="%1$s#comments">%2$s</a></span>',
					esc_url( $item['url'] ),
					sprintf(
						/* translators: %d: Number of post comments. */
						esc_html( _n( '%d Comment', '%d Comments', intval( $item['comments'] ), 'elonix' ) ),
	...
	public static function render_pagination( $data, $settings ) {
		...
		?>
		<nav class="tv-search-results-pagination tv-search-results-pagination-<?php echo esc_attr( $type ); ?>" aria-label="<?php esc_attr_e( 'Search results pagination', 'elonix' ); ?>">
			<?php if ( 'load_more' === $type || 'infinite_scroll' === $type ) : ?>
				<button type="button" class="tv-search-results-load-more" data-next-page="<?php echo esc_attr( $paged + 1 ); ?>" <?php disabled( $paged >= $max_pages ); ?>>
			...
			<?php elseif ( 'numeric' === $type || 'prev_next' === $type ) : ?>
				<?php if ( $paged > 1 ) : ?>
					<a class="tv-search-results-page-link" href="<?php echo esc_url( get_pagenum_link( $paged - 1 ) ); ?>"><?php esc_html_e( 'Prev', 'elonix' ); ?></a>
				<?php endif; ?>
				<?php if ( 'numeric' === $type ) : ?>
					<?php for ( $i = 1; $i <= $max_pages; $i++ ) : ?>
						<a class="tv-search-results-page-link <?php echo $i === $paged ? 'is-active' : ''; ?>" href="<?php echo esc_url( get_pagenum_link( $i ) ); ?>" <?php echo $i === $paged ? 'aria-current="page"' : ''; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
							<?php echo esc_html( $i ); ?>
						</a>
					<?php endfor; ?>
				<?php endif; ?>
				<?php if ( $paged < $max_pages ) : ?>
					<a class="tv-search-results-page-link" href="<?php echo esc_url( get_pagenum_link( $paged + 1 ) ); ?>"><?php esc_html_e( 'Next', 'elonix' ); ?></a>
				<?php endif; ?>
			<?php endif; ?>
		</nav>
		<?php

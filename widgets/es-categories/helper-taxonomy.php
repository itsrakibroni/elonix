...
class Elonix_Toolkit_Taxonomy_Helper {
	...
	private static function format_term_data( $term, $settings ) {
		...
		} elseif ( ( $meta_svg = get_term_meta( $term_id, 'es_cat_svg', true ) ) ) {
			$icon_type  = 'svg';
		...
		} elseif ( ( $meta_image_icon = get_term_meta( $term_id, 'es_cat_image_icon', true ) ) ) {
			$icon_type  = 'image';
			$icon_value = esc_url( $meta_image_icon );
		}
		// 4. Dynamic Term Icon
		elseif ( ( $meta_icon = get_term_meta( $term_id, 'es_cat_icon', true ) ) ) {
			$icon_type  = 'font';
			$icon_value = array(
	...
	private static function resolve_category_image( $term_id, $taxonomy, $settings ) {
		...
		$tax_image_id = get_term_meta( $term_id, 'es_category_image', true );

...
class Elonix_Toolkit_Search_Results_Empty_State_Helper {
	...
	public static function render( $settings, $keyword ) {
		...
		?>
		<section class="es-search-results-empty" aria-labelledby="es-search-results-empty-title">
			<div class="es-search-results-empty-visual" aria-hidden="true">
				<?php if ( ! empty( $settings['empty_lottie_url']['url'] ) ) : ?>
					<lottie-player class="es-search-results-empty-lottie" src="<?php echo esc_url( $settings['empty_lottie_url']['url'] ); ?>" background="transparent" speed="1" loop autoplay></lottie-player>
				<?php else : ?>
					<span class="es-search-results-empty-icon">
			...
			<div class="es-search-results-empty-content">
				<h2 id="es-search-results-empty-title" class="es-search-results-empty-title"><?php echo esc_html( $title ); ?></h2>
				<p class="es-search-results-empty-desc"><?php echo esc_html( $desc ); ?></p>

				<form class="es-search-results-empty-form" role="search" method="get" action="<?php echo esc_url( home_url( '/' ) ); ?>">
					<label class="screen-reader-text" for="es-search-results-empty-field"><?php esc_html_e( 'Search for', 'elonix' ); ?></label>
					<input id="es-search-results-empty-field" type="search" name="s" value="<?php echo esc_attr( $keyword ); ?>" placeholder="<?php esc_attr_e( 'Try another keyword', 'elonix' ); ?>">
					<button type="submit"><?php echo esc_html( $button_text ); ?></button>
				</form>

				<?php self::render_suggestions( $settings, $keyword ); ?>

				<div class="es-search-results-empty-actions">
					<?php if ( $show_home ) : ?>
						<a class="es-search-results-home" href="<?php echo esc_url( home_url( '/' ) ); ?>"><?php esc_html_e( 'Return Home', 'elonix' ); ?></a>
					<?php endif; ?>
					<?php if ( $show_support && ! empty( $settings['support_url']['url'] ) ) : ?>
						<a class="es-search-results-support" href="<?php echo esc_url( $settings['support_url']['url'] ); ?>"><?php esc_html_e( 'Contact Support', 'elonix' ); ?></a>
					<?php endif; ?>
				</div>
			</div>
		</section>
	...
	private static function render_suggestions( $settings, $keyword ) {
		...
		?>
		<div class="es-search-results-suggestions" aria-label="<?php esc_attr_e( 'Search suggestions', 'elonix' ); ?>">
			<?php if ( ( ! isset( $settings['show_popular_searches'] ) || 'yes' === $settings['show_popular_searches'] ) && ! empty( $suggestions ) ) : ?>
				<div class="es-search-results-suggestion-group">

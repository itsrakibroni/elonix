...
class Elonix_Toolkit_Post_Content_Widget extends Elonix_Widget_Base {
	...
	public function get_keywords() {
		...
		return array_merge( $keywords, array( 'post', 'content', 'the_content', 'dynamic', 'tvkit', 'single' ) );
	}
	...
	protected function register_controls() {
		...
		$this->add_responsive_control(
			...
			array(
				...
				'selectors' => array(
					'{{WRAPPER}} .tv-post-content' => 'width: 100%;',
					'{{WRAPPER}} .tv-post-content.width-full' => 'max-width: 100%;',
					'{{WRAPPER}} .tv-post-content.width-boxed' => 'max-width: 1140px; margin-left: auto; margin-right: auto;',
				),
			)
		);
		...
		$this->add_responsive_control(
			...
			array(
				...
				'selectors'  => array(
					'{{WRAPPER}} .tv-post-content.width-custom' => 'max-width: {{SIZE}}{{UNIT}}; margin-left: auto; margin-right: auto;',
				),
			)
		);
		...
		$this->add_responsive_control(
			...
			array(
				...
				'selectors' => array(
					'{{WRAPPER}} .tv-post-content' => 'text-align: {{VALUE}};',
				),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'placeholder' => 'tv-reading-content',
		...
		$scope = '{{WRAPPER}} .tv-post-content.tv-typography-elonix';
		...
		$this->add_responsive_control(
			...
			array(
				...
				'size_units' => array( 'px', 'em', '%' ),
				'selectors'  => array(
					'{{WRAPPER}} .tv-post-content' => 'margin: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				),
			)
		);
		...
		$this->add_responsive_control(
			...
			array(
				...
				'size_units' => array( 'px', 'em', '%' ),
				'selectors'  => array(
					'{{WRAPPER}} .tv-post-content' => 'padding: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				),
			)
		);
	...
	protected function render() {
		...
		try {
			...
			if ( 'tv_single' === get_post_type( $post_id ) ) {
				$preview_id = get_post_meta( $post_id, '_tv_single_preview_id', true );
			...
			// Wrapper Classes
			$wrapper_classes   = array( 'tv-post-content', 'tv-clearfix' );
			$wrapper_classes[] = 'tv-typography-' . $typography_scope;
			...
			if ( 0 === $post_id ) {
				echo '<div class="tv-dummy-content"><p style="padding: 30px; text-align: center; border: 2px dashed #ccc; color: #777;">' . esc_html__( 'Post Content will appear here. Please publish a post to see the preview.', 'elonix' ) . '</p></div>';

...
class Elonix_Toolkit_Search_AJAX_Handler {
	...
	public function __construct() {
		add_action( 'wp_ajax_tv_live_search', array( $this, 'handle_live_search' ) );
		add_action( 'wp_ajax_nopriv_tv_live_search', array( $this, 'handle_live_search' ) );
	}
	...
	public function handle_live_search() {
		...
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( sanitize_key( wp_unslash( $_POST['nonce'] ) ), 'tv_search_nonce' ) ) {
			wp_send_json_error( array( 'message' => esc_html__( 'Security check failed.', 'elonix' ) ), 403 );
		}

		// 2. Validate capability
		if ( ! current_user_can( 'read' ) && ! is_user_logged_in() && 'yes' !== get_option( 'tv_search_allow_guests', 'yes' ) ) {
		...
		$rate_key = 'tv_search_rate_' . md5( $ip );

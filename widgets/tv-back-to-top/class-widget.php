...
class Elonix_Toolkit_Back_To_Top_Widget extends Elonix_Widget_Base {
	...
	public function get_tv_widget_keywords() {
		return array( 'back', 'top', 'scroll', 'progress', 'reading', 'time', 'tvkit' );
	}
	...
	public function get_style_depends() {
		return array( 'elonix-widget-tv-back-to-top' );
	}
	...
	protected function register_controls() {
		...
		$this->add_responsive_control(
			...
			array(
				...
				'selectors'  => array(
					'{{WRAPPER}} .tv-back-to-top-button' => 'width: {{SIZE}}{{UNIT}};',
				),
				'condition'  => array(
					'layout_type!' => 'progress_bar',
				),
		...
		$this->add_responsive_control(
			...
			array(
				...
				'selectors'  => array(
					'{{WRAPPER}} .tv-back-to-top-button' => 'height: {{SIZE}}{{UNIT}};',
				),
				'condition'  => array(
					'layout_type!' => 'progress_bar',
				),
		...
		$this->add_responsive_control(
			...
			array(
				...
				'selectors'  => array(
					'{{WRAPPER}} .tv-back-to-top-button' => 'border-radius: {{TOP}}{{UNIT}} {{RIGHT}}{{UNIT}} {{BOTTOM}}{{UNIT}} {{LEFT}}{{UNIT}};',
				),
				'condition'  => array(
					'layout_type!' => 'progress_bar',
				),
		...
		$this->add_responsive_control(
			...
			array(
				...
				'selectors'  => array(
					'{{WRAPPER}} .tv-back-to-top-container' => 'bottom: {{SIZE}}{{UNIT}};',
				),
				'condition'  => array(
					'layout_type!' => 'progress_bar',
				),
		...
		$this->add_responsive_control(
			...
			array(
				...
				'selectors'  => array(
					'{{WRAPPER}} .tv-align-right' => 'right: {{SIZE}}{{UNIT}};',
					'{{WRAPPER}} .tv-align-left'  => 'left: {{SIZE}}{{UNIT}};',
				),
				'condition'  => array(
					'layout_type!' => 'progress_bar',
				),
		...
		$this->add_control(
			...
			array(
				...
				'selectors' => array(
					'{{WRAPPER}} .tv-bg-glass' => 'backdrop-filter: blur({{SIZE}}px); -webkit-backdrop-filter: blur({{SIZE}}px);',
				),
				'condition' => array(
					'bg_preset' => 'glass',
				),
		...
		$this->add_control(
			...
			array(
				...
				'default'   => '#016087',
				'selectors' => array(
					'{{WRAPPER}} .tv-back-to-top-button' => 'background-color: {{VALUE}};',
					'{{WRAPPER}} .tv-progress-bar-fill'  => 'background-color: {{VALUE}};',
				),
			)
		);
		...
		$this->add_group_control(
			...
			array(
				'name'     => 'button_border',
				'selector' => '{{WRAPPER}} .tv-back-to-top-button',
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'default'   => '#183ad6',
				'selectors' => array(
					'{{WRAPPER}} .tv-back-to-top-button:hover' => 'background-color: {{VALUE}};',
				),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'type'      => Controls_Manager::COLOR,
				'selectors' => array(
					'{{WRAPPER}} .tv-back-to-top-button:hover' => 'border-color: {{VALUE}};',
				),
			)
		);
		...
		$this->add_group_control(
			...
			array(
				...
				'selector' => '{{WRAPPER}} .tv-back-to-top-button:hover',
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'selectors' => array(
					'{{WRAPPER}} .tv-progress-ring-track' => 'stroke: {{VALUE}};',
					'{{WRAPPER}} .tv-progress-track'      => 'background-color: {{VALUE}};',
				),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'default'   => '#3858e9',
				'selectors' => array(
					'{{WRAPPER}} .tv-progress-ring-fill' => 'stroke: {{VALUE}};',
					'{{WRAPPER}} .tv-progress-fill'      => 'background-color: {{VALUE}};',
				),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'selectors' => array(
					'{{WRAPPER}} .tv-progress-ring-fill'  => 'stroke-width: {{SIZE}};',
					'{{WRAPPER}} .tv-progress-ring-track' => 'stroke-width: {{SIZE}};',
					'{{WRAPPER}} .tv-progress-track'      => 'height: {{SIZE}}px;',
				),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'selectors' => array(
					'{{WRAPPER}} .tv-svg-progress-wrapper' => 'padding: {{SIZE}}px;',
				),
				'condition' => array(
		...
		$this->add_control(
			...
			array(
				...
				'default'   => '#ffffff',
				'selectors' => array(
					'{{WRAPPER}} .tv-back-to-top-button'   => 'color: {{VALUE}};',
					'{{WRAPPER}} .tv-back-to-top-icon svg' => 'fill: {{VALUE}};',
					'{{WRAPPER}} .tv-back-to-top-icon-complete svg' => 'fill: {{VALUE}};',
				),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'default'   => '#ffffff',
				'selectors' => array(
					'{{WRAPPER}} .tv-back-to-top-button:hover' => 'color: {{VALUE}};',
					'{{WRAPPER}} .tv-back-to-top-button:hover .tv-back-to-top-icon svg' => 'fill: {{VALUE}};',
					'{{WRAPPER}} .tv-back-to-top-button:hover .tv-back-to-top-icon-complete svg' => 'fill: {{VALUE}};',
				),
			)
		);
		...
		$this->add_responsive_control(
			...
			array(
				...
				'selectors' => array(
					'{{WRAPPER}} .tv-back-to-top-button i' => 'font-size: {{SIZE}}px;',
					'{{WRAPPER}} .tv-back-to-top-button svg' => 'width: {{SIZE}}px; height: {{SIZE}}px;',
				),
			)
		);
	...
	protected function render() {
		...
		$align_class = 'tv-align-right';
		if ( isset( $settings['position_align'] ) ) {
			$align_class = 'tv-align-' . esc_attr( $settings['position_align'] );
		}

		// Sticky positioning class
		$sticky_class = '';
		if ( isset( $settings['position_sticky'] ) ) {
			$sticky_class = 'tv-' . esc_attr( $settings['position_sticky'] );
		}

		// CSS Preset classes
		$preset_class = 'tv-bg-' . esc_attr( $settings['bg_preset'] );
		$layout_class = 'tv-layout-' . esc_attr( $settings['layout_type'] );
		$motion_class = 'tv-motion-in-' . esc_attr( $settings['entrance_animation'] );
		$hover_class  = 'tv-motion-hover-' . esc_attr( $settings['hover_transition'] );

		// Neumorphic inline style variables
		$style_attrs = '';
		if ( 'neumorphism' === $settings['bg_preset'] && isset( $settings['neumorphism_angle_val']['size'] ) ) {
			$style_attrs = ' style="--tv-neumorphic-angle: ' . (int) $settings['neumorphism_angle_val']['size'] . 'deg;"';
		}
		...
		if ( 'progress_bar' === $settings['layout_type'] ) {
			...
			$bar_class = 'tv-progress-bar-' . esc_attr( $bar_pos );
			?>
			<div class="tv-progress-track tv-progress-bar-track <?php echo esc_attr( $bar_class ); ?>" data-config="<?php echo esc_attr( wp_json_encode( $js_config ) ); ?>">
				<div class="tv-progress-fill tv-progress-bar-fill"></div>
			</div>
			<?php
			return;
		}
		...
		?>
		<div class="tv-back-to-top-container <?php echo esc_attr( $align_class ); ?> <?php echo esc_attr( $motion_class ); ?> <?php echo esc_attr( $sticky_class ); ?>" data-config="<?php echo esc_attr( wp_json_encode( $js_config ) ); ?>">
			
			<button class="tv-back-to-top-button <?php echo esc_attr( $layout_class ); ?> <?php echo esc_attr( $preset_class ); ?> <?php echo esc_attr( $hover_class ); ?>" 
				...
				<?php if ( 'circular' === $settings['progress_mode'] || 'progress_ring' === $settings['layout_type'] ) : ?>
					<div class="tv-svg-progress-wrapper">
						<svg class="tv-progress-svg" viewBox="0 0 100 100" aria-hidden="true">
							<circle class="tv-progress-ring-track" cx="50" cy="50" r="45"></circle>
							<circle class="tv-progress-ring-fill" cx="50" cy="50" r="45"></circle>
						</svg>
					</div>
				<?php endif; ?>

				<?php if ( 'linear' === $settings['progress_mode'] ) : ?>
					<div class="tv-progress-track tv-linear-progress-bar-track">
						<div class="tv-progress-fill tv-linear-progress-bar-fill"></div>
					</div>
				<?php endif; ?>

				<div class="tv-button-content-wrapper">
					<?php
					// Dynamic Icon/Graphics renderer
					$icon_swappable = ( 'yes' === $settings['enable_reading_assistant'] && 'yes' === $settings['enable_icon_swap'] );
					?>
					
					<span class="tv-back-to-top-icon <?php echo $icon_swappable ? 'tv-swappable' : ''; ?>">
						<?php
						...
						if ( 'library' === $icon_source ) {
							...
							echo '<img class="tv-svg-icon" src="' . esc_url( $settings['selected_svg']['url'] ) . '" alt="" aria-hidden="true" />';
						} elseif ( 'image' === $icon_source && ! empty( $settings['selected_image']['url'] ) ) {
							echo '<img class="tv-img-icon" src="' . esc_url( $settings['selected_image']['url'] ) . '" alt="" aria-hidden="true" />';
						}
						?>
					</span>

					<?php if ( $icon_swappable ) : ?>
						<span class="tv-back-to-top-icon-complete" style="display: none;">
							<?php Icons_Manager::render_icon( $settings['completion_icon'], array( 'aria-hidden' => 'true' ) ); ?>
						</span>
					<?php endif; ?>

					<?php if ( 'percent' === $settings['progress_mode'] ) : ?>
						<span class="tv-progress-percent-text">0%</span>
					<?php endif; ?>

					<?php if ( 'yes' === $settings['enable_reading_assistant'] && 'none' !== $settings['reading_assistant_mode'] ) : ?>
						<span class="tv-reading-time-text"></span>
					<?php endif; ?>
				</div>

...
class Elonix_Toolkit_Post_List_AJAX {
	...
	public function __construct() {
		add_action( 'wp_ajax_tv_post_list_fetch_posts', array( $this, 'ajax_fetch_posts' ) );
		add_action( 'wp_ajax_nopriv_tv_post_list_fetch_posts', array( $this, 'ajax_fetch_posts' ) );
	}
	...
	public function ajax_fetch_posts() {
		...
		check_ajax_referer( 'tv-post-list-nonce', 'security' );
		...
		if ( $query->have_posts() ) {
			...
			while ( $query->have_posts() ) {
				...
				$item = array(
					...
					'views'        => intval( get_post_meta( $post->ID, 'tv_post_views_count', true ) ),

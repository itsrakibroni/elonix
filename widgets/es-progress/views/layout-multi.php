...
<div class="es-progress__wrapper">
	...
	<div class="es-progress__track">
		...
		foreach ( $items as $index => $item ) :
			...
			?>
		<div class="es-progress__segment elementor-repeater-item-<?php echo esc_attr( $item['_id'] ); ?>" 
			style="--es-segment-value: <?php echo esc_attr( $percent ); ?>%; <?php echo $color_style; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>"
			title="<?php echo esc_attr( $item['title'] . ': ' . $item['current_value'] ); ?>">
		</div>
		<?php endforeach; ?>
	</div>
	
	<div class="es-progress__legend">
		<?php
		foreach ( $items as $index => $item ) :
			$color_style = ! empty( $item['item_color'] ) ? 'background-color: ' . esc_attr( $item['item_color'] ) . ';' : '';
			?>
		<div class="es-progress__legend-item">
			<span class="es-progress__legend-color" style="<?php echo $color_style; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>"></span>
			<span class="es-progress__legend-title"><?php echo esc_html( $item['title'] ); ?></span>
			<span class="es-progress__legend-value">(<?php echo esc_html( $item['current_value'] ); ?>)</span>
		</div>
		<?php endforeach; ?>
	</div>
</div>

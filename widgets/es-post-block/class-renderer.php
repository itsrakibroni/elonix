...
class Elonix_Toolkit_Post_Block_Renderer {
	...
	public static function render_single_post( $item, $settings, $layout, $post_index = 0 ) {
		$card_classes   = array( 'es-post-block-card' );
		...
		if ( 'style_3' === $layout ) {
			if ( 0 === $post_index ) {
				$card_classes[] = 'es-grid-primary';
			} else {
				$card_classes[] = 'es-grid-secondary';
			}
		}
	...
	protected static function render_element_block( $type, $item, $settings, $block = array() ) {
		switch ( $type ) {
			...
			case 'title':
				...
				printf(
					'<%1$s class="es-post-block-title"><a href="%2$s">%3$s</a></%1$s>',
					esc_attr( $title_tag ),
					esc_url( $item['url'] ),
					esc_html( $title_val )
				);
			...
			case 'meta':
				...
				if ( ! empty( $meta_order ) ) {
					echo '<div class="es-post-block-meta">';
			...
			case 'excerpt':
				...
				if ( ! empty( $excerpt_val ) ) {
					printf(
						'<div class="es-post-block-excerpt">%s</div>',
			...
			case 'author':
				...
				echo '<div class="es-post-block-meta-single' . esc_attr( $class_suffix ) . '">';
			...
			case 'share':
				...
				$class_suffix  = ( 'after' === $icon_position ) ? ' es-icon-pos-after' : '';
				if ( ! empty( $block['_id'] ) ) {
					$class_suffix .= ' elementor-repeater-item-' . $block['_id'];
				}

				$icon_markup  = self::get_icon_markup( $settings, 'share_icon' );
				$label_markup = '<span class="es-share-label-text">' . esc_html__( 'Share:', 'elonix' ) . '</span>';
				?>
				<div class="es-post-block-share<?php echo esc_attr( $class_suffix ); ?>">
					...
					<a href="<?php echo esc_url( 'https://api.whatsapp.com/send?text=' . rawurlencode( $item['url'] ) ); ?>" target="_blank" rel="noopener" class="es-share-btn wa" aria-label="<?php esc_attr_e( 'Share on WhatsApp', 'elonix' ); ?>">W</a>
				</div>
				<?php
				break;
		}
	...
	protected static function render_smart_badges( $item, $settings ) {
		...
		if ( 'yes' === $show_new && ! empty( $item['is_new'] ) ) {
			$badge_list[] = '<span class="es-sbadge es-sbadge-new">' . esc_html__( 'New', 'elonix' ) . '</span>';
		}
		if ( 'yes' === $show_popular && ! empty( $item['is_popular'] ) ) {
			$badge_list[] = '<span class="es-sbadge es-sbadge-popular">' . esc_html__( 'Popular', 'elonix' ) . '</span>';
		}
		if ( 'yes' === $show_trending && ! empty( $item['is_trending'] ) ) {
			$badge_list[] = '<span class="es-sbadge es-sbadge-trending">' . esc_html__( 'Trending', 'elonix' ) . '</span>';
		}
		if ( 'yes' === $show_sponsored && ! empty( $item['is_sponsored'] ) ) {
			$badge_list[] = '<span class="es-sbadge es-sbadge-sponsored">' . esc_html__( 'Sponsored', 'elonix' ) . '</span>';
		}
	...
	protected static function get_icon_markup( $settings, $icon_key ) {
		if ( isset( $settings[ $icon_key ] ) && ! empty( $settings[ $icon_key ]['value'] )
			...
			ob_start();
			echo '<span class="es-post-block-icon">';

...
class Elonix_Toolkit_Post_Block_Query_Helper {
	...
	public static function format_post_data( $post, $settings ) {
		...
		return array(
			...
			'is_sponsored'      => 'yes' === get_post_meta( $post->ID, 'tv_is_sponsored', true ),
			'is_verified'       => 'yes' === get_post_meta( $post->ID, 'tv_is_verified', true ) || 'yes' === get_user_meta( $post->post_author, 'tv_author_verified', true ),
			'is_editors_choice' => 'yes' === get_post_meta( $post->ID, 'tv_editors_choice', true ),
		);
	}

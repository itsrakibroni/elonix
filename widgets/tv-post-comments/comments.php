...
<div id="comments" class="tv-comments-area">
	
	<?php if ( $has_comments ) : ?>
		
		<?php if ( 'yes' === $settings['show_comment_count'] ) : ?>
			<h3 class="tv-comments-title">
				<?php
				if ( 1 === $total_comments ) {
		...
		<?php if ( 'yes' === $settings['show_pagination'] && ! empty( $pagination_links ) ) : ?>
			<div class="tv-comment-navigation">
				<?php echo wp_kses_post( $pagination_links ); ?>
			</div>
		<?php endif; ?>

		<?php if ( ! comments_open() ) : ?>
			<p class="tv-no-comments"><?php echo esc_html( $settings['empty_comments_message'] ); ?></p>
		<?php endif; ?>
	...
	if ( 'yes' === $settings['show_comment_form'] && comments_open() ) :
		$args = array(
			...
			'class_container'   => 'tv-comment-respond',
			'class_form'        => 'tv-comment-form',
			'class_submit'      => 'tv-submit-btn',
		);
		comment_form( $args );

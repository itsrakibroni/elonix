...
class Elonix_Toolkit_Popup_AJAX {
	...
	public function __construct() {
		...
		add_action( 'wp_ajax_tv_duplicate_popup', array( $this, 'duplicate_popup_template' ) );

		// Add quick duplicate links inside post rows in list table
	}

	/**
	 * Inject dynamic "Duplicate" link inside row actions of tv_popup.
	 */
	public function inject_duplicate_link( $actions, $post ) {
		if ( 'tv_popup' === $post->post_type ) {
			$nonce                   = wp_create_nonce( 'tv_duplicate_popup_' . $post->ID );
			$url                     = admin_url( 'admin-ajax.php?action=tv_duplicate_popup&post_id=' . $post->ID . '&nonce=' . $nonce );
	...
	public function duplicate_popup_template() {
		...
		if ( ! $post_id || ! wp_verify_nonce( $nonce, 'tv_duplicate_popup_' . $post_id ) ) {
		...
		if ( ! $post || 'tv_popup' !== $post->post_type ) {
		...
		$meta_keys = array(
			'_tv_popup_type',
			'_tv_popup_trigger_type',
			'_tv_popup_trigger_value',
			'_tv_popup_target_rule',
			'_tv_popup_target_ids',
			'_tv_popup_devices',
			'_tv_popup_frequency',
			'_tv_popup_cookie_expiry',
			'_elementor_data',
		...
		wp_safe_redirect( admin_url( 'edit.php?post_type=tv_popup' ) );
		exit;
	}
}
